...
{name:"Alimentação",stems:["Eu quero {x}","Eu gosto de {x}","Eu não quero {x}","Tem {x}","Onde posso comprar {x}","Pode trazer {x}","Quero pedir {x}","Preciso preparar {x}","Você quer {x}","Vamos fazer {x}"],items:["água","café","chá","suco","leite","pão","arroz","feijão","carne","frango","peixe","salada","sopa","macarrão","pizza","sanduíche","bolo","fruta","banana","maçã","laranja","açúcar","sal","sobremesa","o almoço"]},
{name:"Estudo e trabalho",stems:["Preciso de {x}","Quero revisar {x}","Pode explicar {x}","Onde está {x}","Vamos começar {x}","Preciso terminar {x}","Quero organizar {x}","Pode enviar {x}","Vamos conversar sobre {x}","Preciso conferir {x}"],items:["a atividade","a aula","o exercício","o caderno","o livro","a prova","o trabalho","o relatório","o arquivo","o documento","a planilha","a apresentação","o computador","o teclado","o mouse","a impressora","o projeto","a reunião","o prazo","o horário","a tarefa","a resposta","a pesquisa","o conteúdo","o material"]},
{name:"Cidade e transporte",stems:["Quero ir para {x}","Como chego a {x}","Onde fica {x}","Preciso encontrar {x}","Você sabe onde é {x}","Pode me levar até {x}","Qual caminho vai para {x}","Estou indo para {x}","Vamos até {x}","Quanto tempo leva até {x}"],items:["o centro","a rodoviária","o aeroporto","a estação","o ponto de ônibus","o metrô","a praça","o shopping","o mercado","a farmácia","o hospital","a escola","a universidade","o banco","a prefeitura","o restaurante","o hotel","a praia","o parque","o museu","a biblioteca","a igreja","o trabalho","minha casa","a avenida principal"]},
{name:"Saúde e bem-estar",stems:["Preciso falar sobre {x}","Estou sentindo {x}","Quero verificar {x}","Preciso cuidar de {x}","Pode me explicar {x}","Quero marcar {x}","Preciso informar {x}","Estou preocupado com {x}","Quero melhorar {x}","Preciso de ajuda com {x}"],items:["uma dor de cabeça","uma dor nas costas","uma dor no braço","uma dor na perna","uma dor de garganta","uma consulta","um exame","um remédio","uma receita","minha pressão","minha alimentação","meu sono","meu cansaço","meu estresse","minha respiração","uma alergia","uma febre","uma tontura","um machucado","uma vacina","meu tratamento","minha recuperação","uma emergência","minha saúde","meu bem-estar"]},
{name:"Compras e serviços",stems:["Quero saber o preço de {x}","Quero comprar {x}","Preciso trocar {x}","Onde encontro {x}","Tem desconto em {x}","Posso pagar {x}","Quero comparar {x}","Pode separar {x}","Preciso de informação sobre {x}","Quero receber {x}"],items:["este produto","esta roupa","este sapato","esta bolsa","este celular","este computador","este livro","este presente","este pedido","esta compra","a entrega","a garantia","a nota fiscal","o pagamento","o cartão","o dinheiro","o desconto","o tamanho","a cor","o modelo","o atendimento","o orçamento","o serviço","a assinatura","a encomenda"]},
