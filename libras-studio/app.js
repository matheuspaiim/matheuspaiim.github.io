...
async function uploadVideo(f){if(!f)return;let names=$("#create-name").value.split(/\n/).map(x=>x.trim()).filter(Boolean);if(!names.length)return toast("Digite o nome antes.");if(names.length>1)return toast("Para enviar um vídeo, deixe apenas um sinal no campo.");let n=names[0];let path=S.session.user.id+"/"+Date.now()+"_"+f.name.replace(/[^a-zA-Z0-9._-]/g,"_"),up=await sb.storage.from("sign-videos").upload(path,f);if(up.error)return toast("Falha no upload.");let row={user_id:S.session.user.id,studio_id:uuid(),name:title(n),norm:norm(n),variant_no:S.signs.filter(x=>x.norm===norm(n)).length+1,variant_key:"upload|"+path,category_name:catFor(n),category_source:"auto",category_tags:"[]",source_id:"mobile_upload",source_name:"Vídeo meu",origin:"mobile_upload",media_path:path,updated_at:new Date().toISOString()},{data,error}=await sb.from("signs").insert(row).select().single();if(error)return toast(error.message);let z=await sb.storage.from("sign-videos").createSignedUrl(path,86400);data.media_url=z.data?.signedUrl;S.signs.push(data);cache();render();toast("Vídeo enviado ☁️")}
