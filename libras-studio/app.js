...
async function sync({silent=false}={}){
  if(!S.session)return;
  cached();render();
  if(!navigator.onLine){localStatus("📴 offline · dados locais");return}
  if(window.__lsSyncing)return;
  window.__lsSyncing=true;
  if(!silent)localStatus("sincronizando…");
  ...
  try{
    ...
    cache();render();localStatus("☁️ sincronizado");
    signMedia().then(()=>{cache();render();cacheLibraryVideos()}).catch(e=>console.warn("mídias",e));
  }catch(e){
    console.error("sync",e);cached();localStatus("📴 offline · dados locais");render();
...
function renderProgressPage(){
  ...
  let moduleIcons=["📘","👁️","↔️","💬"];
  let modules=snap.moduleRows.map((x,i)=>{let pct=Math.round((x.done/Math.max(1,x.total))*100);return '<div class="module-progress-row tone-'+(i%4)+'"><div class="module-progress-icon"><span>'+moduleIcons[i%moduleIcons.length]+'</span></div><div class="module-progress-copy"><div class="module-progress-head"><b>'+esc(moduleLabel(x.name))+'</b><strong>'+pct+'%</strong></div><small>'+x.done+' de '+x.total+' aulas concluídas</small><div class="module-progress-track"><span style="width:'+pct+'%"></span></div></div></div>'}).join("");
...
async function uploadVideo(f){if(!f)return;let names=$("#create-name").value.split(/\n/).map(x=>x.trim()).filter(Boolean);if(!names.length)return toast("Digite o nome antes.");if(names.length>1)return toast("Para enviar um vídeo, deixe apenas um sinal no campo.");let n=names[0];let path=S.session.user.id+"/"+Date.now()+"_"+f.name.replace(/[^a-zA-Z0-9._-]/g,"_"),up=await sb.storage.from("sign-videos").upload(path,f);if(up.error)return toast("Falha no upload.");let row={user_id:S.session.user.id,studio_id:uuid(),name:title(n),norm:norm(n),variant_no:S.signs.filter(x=>x.norm===norm(n)).length+1,variant_key:"upload|"+path,category_name:catFor(n),category_source:"auto",category_tags:"[]",source_id:"mobile_upload",source_name:"Vídeo meu",origin:"mobile_upload",media_path:path,updated_at:new Date().toISOString()},{data,error}=await sb.from("signs").insert(row).select().single();if(error)return toast(error.message);let z=await sb.storage.from("sign-videos").createSignedUrl(path,86400);data.media_url=z.data?.signedUrl;S.signs.push(data);cache();render();cacheLibraryVideo(data);toast("Vídeo enviado ☁️")}
...
async function saveLesson(id,v){let row={user_id:S.session.user.id,lesson_id:id,completed:v,completed_at:v?new Date().toISOString():null,updated_at:new Date().toISOString()},i=S.study.findIndex(x=>x.lesson_id===id);if(i>=0)S.study[i]=row;else S.study.push(row);let L=window.LIBRAS_STUDY_CONTENT||[],idx=L.findIndex(x=>x.id===id);if(v){let nx=L.slice(idx+1).find(x=>!smap()[x.id]?.completed);if(nx)localStorage.setItem("ls-last-lesson",nx.id)}else localStorage.setItem("ls-last-lesson",id);cache();render();if(!navigator.onLine){queuePending("study",id,row);localStatus("📴 offline · alterações salvas");return}try{let{error}=await sb.from("study_progress").upsert(row,{onConflict:"user_id,lesson_id"});if(error)throw error}catch(e){console.warn("progresso offline",e);queuePending("study",id,row);localStatus("📴 salvo no aparelho")}}
...
async function moveLibraryItem(studioId,category){
  let item=S.signs.find(x=>x.studio_id===studioId);if(!item||!category||item.category_name===category)return;
  let old=item.category_name,patch={category_name:category,category_source:"manual",updated_at:new Date().toISOString()};Object.assign(item,patch);cache();library();
  if(!navigator.onLine){queuePending("sign_update",studioId,patch);localStatus("📴 alteração salva");toast("Movido para "+category);return}
  try{let{error}=await sb.from("signs").update(patch).eq("studio_id",studioId);if(error)throw error;toast("Movido para "+category)}catch(e){console.error(e);item.category_name=old;cache();library();toast("Não consegui mover o sinal.")}
}
async function deleteLibraryItem(studioId){
  let item=S.signs.find(x=>x.studio_id===studioId);if(!item||!confirm("Excluir "+item.name+" da biblioteca?"))return;
  let now=new Date().toISOString(),patch={deleted_at:now,updated_at:now};S.signs=S.signs.filter(x=>x.studio_id!==studioId);deleteCachedLibraryVideo(item);cache();render();
  if(!navigator.onLine){queuePending("sign_delete",studioId,patch);localStatus("📴 exclusão salva");return toast("Sinal removido da biblioteca.")}
...
function reviewCats(){function reviewCats(){let s=$("#review-cat"),v=s.value,cs=[...new Set(S.signs.map(x=>x.category_name).filter(Boolean))].sort();s.innerHTML='<option value="">Todas</option>'+cs.map(x=>'<option>'+esc(x)+"</option>").join("");s.value=v}
...
async function rate(r){let s=S.queue[S.i],row={user_id:S.session.user.id,norm:s.norm,...next(r,rmap()[s.norm]),last_rating:r,last_reviewed_at:new Date().toISOString(),updated_at:new Date().toISOString()},i=S.reviews.findIndex(x=>x.norm===s.norm);if(i>=0)S.reviews[i]=row;else S.reviews.push(row);S.i++;cache();render();card();if(!navigator.onLine){queuePending("review",s.norm,row);localStatus("📴 offline · revisão salva");return}try{let{error}=await sb.from("review_state").upsert(row,{onConflict:"user_id,norm"});if(error)throw error}catch(e){console.warn("revisão offline",e);queuePending("review",s.norm,row);localStatus("📴 salvo no aparelho")}}
...
async function init(){
  ...
  sb.auth.onAuthStateChange((event,session)=>{
    if(session){
      ...
      if(navigator.onLine)setTimeout(()=>sync({silent:true}),0);
  ...
  try{
    let{data:{session},error}=await sb.auth.getSession();
    if(error)throw error;
    if(session){S.session=session;rememberSession(session);cached(session.user.id);authUI();if(navigator.onLine)sync({silent:true});else localStatus("📴 offline · dados locais")}
  ...
  window.addEventListener("online",()=>{localStatus("☁️ reconectando…");revalidateSessionAndSync()});
