...
async function refreshLibrasLab(){applyEditionUI();let warn=$("#ll-not-native"),statusCard=$("#ll-status-card"),sessionCard=$("#ll-session-card"),detCard=$("#ll-detected-card");if(!warn||!statusCard)return;let native=personalEditionAvailable();warn.classList.toggle("hidden",native);statusCard.classList.toggle("hidden",!native);sessionCard.classList.toggle("hidden",!native);if(!native){detCard.classList.add("hidden");return}let st=getNativeState()||{},enabled=!!st.accessibilityEnabled,live=!!st.captureEnabled;$("#ll-accessibility-status").textContent=enabled?"Conectado ao LibrasLab":"Permissão ainda não ativada";$("#ll-accessibility-dot").className="status-dot "+(live?"live":enabled?"ok":"");$("#ll-enable").classList.toggle("hidden",enabled);$("#ll-start").classList.toggle("hidden",live);$("#ll-start").disabled=!enabled;$("#ll-open").classList.toggle("hidden",!live);$("#ll-stop").classList.toggle("hidden",!live);let raw=Array.isArray(st.detections)?st.detections:[];$("#ll-session-info").textContent=live?"🟣 Captura ativa · "+raw.length+" respostas tocadas · "+elapsedText(st.sessionStartedAt):raw.length?"Aula finalizada. Revise os sinais detectados abaixo.":"Inicie a captura antes de abrir o LibrasLab.";if(!raw.length){detCard.classList.add("hidden");S.librasLabCandidates=[];return}detCard.classList.remove("hidden");$("#ll-detected-list").innerHTML='<div class="ll-progress">🔎 Conferindo '+raw.length+' respostas com o catálogo do Studio…</div>';let candidates=await buildLibrasLabCandidates(raw);S.librasLabCandidates=candidates;$("#ll-detected-title").textContent=candidates.length+" "+(candidates.length===1?"sinal":"sinais");$("#ll-detected-meta").textContent=raw.length+" respostas tocadas no LibrasLab · "+candidates.length+" reconhecidas como sinais no catálogo.";$("#ll-detected-list").innerHTML=candidates.length?candidates.map((c,i)=>'<label class="ll-candidate '+(c.existing?"existing":"")+'"><input type="checkbox" data-ll-index="'+i+'" '+(c.uiLikely?"":"checked")+'><div><b>'+esc(c.name)+'</b><small>'+c.count+' ocorrência(s) · '+(c.existing?"já está na biblioteca":"novo sinal")+(c.uiLikely?" · pode ser botão da interface":"")+'</small></div>'+(c.existing?'<span class="ll-badge">já salvo</span>':"")+'</label>').join(""):'<div class="ll-empty">Ainda não reconheci nomes de sinais nesta sessão. Continue a aula e toque em ↻ Atualizar.</div>'}
